...
function authomaton(text, pattern) {
    let start = process.hrtime.bigint(); //запоминаем начало времени
    let m = pattern.length;
    let alphabet = [...new Set(pattern)]; //создаем коллекцию уникальных символов шаблона и превращаем ее в массив, оператор ... spread разворачивает элементы множества
    let matches = [];
    let comp = 0;
    let table = Array(m + 1).fill().map(() => ({}));
    //создали массив длиной m+1, с помощью .fill заполнили его undefined (иначе не сработает map)
    //с помощью map преобразовали каждый элемент в пустой объект {}
    
    for (let state = 0; state <= m; state++) {
        for (let char of alphabet) {
            //пробуем перейти в максимально возможное состояние, либо state+1, либо m (если вышли за длину шаблона)
            let nextState = Math.min(state + 1, m);
            // Поиск правильного перехода. уменьшаем nextState, пока не найдём подходящий
            while (nextState > 0) {
                if (char === pattern[nextState - 1]) { //смотрим, совпадает ли символ с нужным в шаблоне
                    let match = true;
                    for (let i = 0; i < nextState - 1; i++) {
                        if (pattern[i] !== pattern[state - nextState + 1 + i]) { //сравниваем часть уже найденного шаблона
                            match = false;
                            break;
                        }
                    }
                    if (match) break; //если и символ, и префикс совпали - переход возможен
                }
                nextState--; //если нет - пробуем меньшее состояние
            }
            
            //записываем переход в таблицу,из текущего state по символу char -> nextState
            table[state][char] = nextState;
        }
    }
    let currentState = 0; //начинаем поиск вхождений с начального состояния
    for (let i = 0; i < text.length; i++) { //идем по тексту
        let char = text[i];
        comp++; //считаем раз, в который сравнивали символы
        currentState = table[currentState][char] || 0; //переходим в следующее состояние по таблице или в 0, если некорректный символ
        if (currentState === m) { //достигли конечного состояния -> нашли совпадение
            matches.push(i - m + 1); //кладем в массив позицию начала совпадения
        }
    }
